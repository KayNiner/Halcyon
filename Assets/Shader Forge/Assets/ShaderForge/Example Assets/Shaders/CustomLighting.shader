...
Shader "Shader Forge/Examples/Custom Lighting" {
    Properties {
        ...
        _OutlineWidth ("OutlineWidth", Float ) = 0.001
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
////// Lighting:
                ...
                float3 finalColor = (i.vertexColor.rgb*(attenuation*_LightColor0.rgb*((_Diffuse_var.rgb*_Color_var.rgb*floor(max(0,dot(lightDirection,normalDirection)) * _Bands_var) / (_Bands_var - 1))+UNITY_LIGHTMODEL_AMBIENT.rgb+floor(pow(max(0,dot(normalDirection,halfDirection)),exp2(((_Gloss_var*10.0)+1.0))) * _Bands_var) / (_Bands_var - 1))));
